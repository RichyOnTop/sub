...
class Config:
    ...
    # Full calibration stages (multi-stage guided calibration)
    CALIB_STAGES = [
        "STAND STILL",
        "RAISE ARMS OVER SHOULDERS (JUMP)",
        "CROUCH DOWN (DUCK)",
        "LEAN QUICKLY RIGHT",
        "LEAN QUICKLY LEFT",
    ]
    CALIB_STAGE_DURATION = 5.0  # seconds per stage

    # Timing
    KEY_TAP_DURATION = 0.10
    ACTION_COOLDOWN = 0.30
    STABILITY_FRAMES = 3    # slightly faster response for quick faint
...
class SmoothedLandmarks:
    ...
    def __init__(self, alpha=0.25):
        ...
        self._prev_shoulder_cx = None
        self._shoulder_cx_velocity = 0.0
        self._ready = False
    ...
    def update(self, pose_landmarks_list):
        ...
        # Compute shoulder center velocity (for quick lean/faint detection)
        shoulder_cx_now = (self._smoothed[11][0] + self._smoothed[12][0]) / 2.0
        if self._prev_shoulder_cx is not None and self._prev_time is not None:
            dt = now - self._prev_time
            if dt > 0.005:
                self._shoulder_cx_velocity = (shoulder_cx_now - self._prev_shoulder_cx) / dt
        self._prev_shoulder_cx = shoulder_cx_now
    ...
    @property
    def shoulder_y(self):
        if not self._smoothed or len(self._smoothed) < 13:
            return 0.5
        return (self._smoothed[11][1] + self._smoothed[12][1]) / 2.0

    @property
    def left_elbow_y(self):
        if not self._smoothed or len(self._smoothed) < 14:
            return 0.5
        return self._smoothed[13][1]

    @property
    def right_elbow_y(self):
        if not self._smoothed or len(self._smoothed) < 15:
            return 0.5
        return self._smoothed[14][1]

    @property
    def min_elbow_y(self):
        return min(self.left_elbow_y, self.right_elbow_y)

    @property
    def shoulder_cx_velocity(self):
        return self._shoulder_cx_velocity
...
class GestureInterpreter:
    ...
    Full multi-stage calibration: stand, jump (arms), duck, lean right, lean left.
    Thresholds are set dynamically from calibrated movement ranges.
    """
    def __init__(self, config):
        ...
        self.calibration_stages = config.CALIB_STAGES
        self.calibration_stage_duration = config.CALIB_STAGE_DURATION
        self.baseline_y = None
        self.baseline_shoulder_cx = None
        self.baseline_shoulder_y = None
        self.baseline_hip_cy = None

        self._calib_stage_index = 0
        self._calib_start = None
        self._calib_start_stage = None
        self._calib_active = False
        self._calib_samples = {}

        # Dynamic thresholds (calibrated after full calibration)
        self.thresholds = {
            'jump_velocity': config.JUMP_VELOCITY_THRESHOLD,
            'jump_position': config.JUMP_POSITION_THRESHOLD,
            'jump_arm_raise': 0.03,
            'duck_position': config.DUCK_POSITION_THRESHOLD,
            'duck_hip_factor': 0.5,
            'lean_right': config.LEAN_THRESHOLD,
            'lean_left': config.LEAN_THRESHOLD,
            'lean_velocity': 0.05,
        }
    ...
    def start_calibration(self):
        """Call this to (re-)start full multi-stage calibration."""
        self.baseline_y = None
        self.baseline_shoulder_cx = None
        self.baseline_shoulder_y = None
        self.baseline_hip_cy = None
        ...
        self._calib_start_stage = time.time()
        self._calib_active = True
        self._calib_samples = {}
        print("[Calib] Multi-stage calibration started.")
        print("[Calib] Stage 1/{} — {}".format(
            len(self.calibration_stages), self.calibration_stages[0]))

    def _update_calibration(self, head_y, shoulder_cx, shoulder_y, hip_cy, min_elbow_y):
        """Collect samples for the current calibration stage."""
        if not self._calib_active:
            return

        stage_name = self.calibration_stages[self._calib_stage_index]
        samples = self._calib_samples.get(stage_name, [])
        samples.append({
            'head_y': head_y,
            'shoulder_cx': shoulder_cx,
            'shoulder_y': shoulder_y,
            'hip_cy': hip_cy,
            'min_elbow_y': min_elbow_y,
        })
        self._calib_samples[stage_name] = samples

        stage_elapsed = time.time() - self._calib_start_stage
        if stage_elapsed >= self.calibration_stage_duration:
            # Move to next stage
            self._calib_stage_index += 1
            self._calib_start_stage = time.time()
            if self._calib_stage_index >= len(self.calibration_stages):
                # All stages complete — finalize
                self._finalize_calibration()
                self._calib_active = False
                print("[Calib] All stages complete. Calibration locked.")
            else:
                print("[Calib] Stage {}/{} — {}".format(
                    self._calib_stage_index + 1,
                    len(self.calibration_stages),
                    self.calibration_stages[self._calib_stage_index]))

    def _finalize_calibration(self):
        """Compute all references and dynamic thresholds from collected samples."""
        # Stand references (stage 0)
        stand_samples = self._calib_samples.get(self.calibration_stages[0], [])
        if stand_samples:
            self.baseline_y = float(np.mean([s['head_y'] for s in stand_samples]))
            self.baseline_shoulder_cx = float(np.mean([s['shoulder_cx'] for s in stand_samples]))
            self.baseline_shoulder_y = float(np.mean([s['shoulder_y'] for s in stand_samples]))
            self.baseline_hip_cy = float(np.mean([s['hip_cy'] for s in stand_samples]))

        # Jump references (stage 1)
        jump_samples = self._calib_samples.get(self.calibration_stages[1], [])
        jump_shoulder_rise = 0.0
        jump_arm_raise = 0.0
        if jump_samples and stand_samples:
            # Shoulder rise: lower Y = higher shoulder
            shoulder_rises = [self.baseline_shoulder_y - s['shoulder_y'] for s in jump_samples]
            jump_shoulder_rise = max(shoulder_rises) if shoulder_rises else 0.0
            # Arm raise: shoulder Y - min elbow Y > 0 means elbow above shoulder
            arm_raises = [s['shoulder_y'] - s['min_elbow_y'] for s in jump_samples]
            jump_arm_raise = max(arm_raises) if arm_raises else 0.0

        # Duck references (stage 2)
        duck_samples = self._calib_samples.get(self.calibration_stages[2], [])
        duck_head_drop = 0.0
        duck_hip_drop = 0.0
        if duck_samples and stand_samples:
            head_drops = [s['head_y'] - self.baseline_y for s in duck_samples]
            hip_drops = [s['hip_cy'] - self.baseline_hip_cy for s in duck_samples]
            duck_head_drop = max(head_drops) if head_drops else 0.0
            duck_hip_drop = max(hip_drops) if hip_drops else 0.0

        # Lean references (stage 3 and 4)
        lean_right_shift = 0.0
        lean_left_shift = 0.0
        lean_r_samples = self._calib_samples.get(self.calibration_stages[3], [])
        lean_l_samples = self._calib_samples.get(self.calibration_stages[4], [])
        if lean_r_samples and stand_samples:
            lean_right_shift = max([s['shoulder_cx'] - self.baseline_shoulder_cx for s in lean_r_samples])
        if lean_l_samples and stand_samples:
            lean_left_shift = max([self.baseline_shoulder_cx - s['shoulder_cx'] for s in lean_l_samples])

        print("[Calib] References — shoulder_rise={:.3f} arm_raise={:.3f} head_drop={:.3f} lean_r={:.3f} lean_l={:.3f}".format(
            jump_shoulder_rise, jump_arm_raise, duck_head_drop, lean_right_shift, lean_left_shift))

        # Dynamic thresholds (fraction of calibrated max, clamped to sensible ranges)
        self.thresholds['jump_velocity'] = self.cfg.JUMP_VELOCITY_THRESHOLD
        self.thresholds['jump_position'] = max(0.02, min(0.08, jump_shoulder_rise * 0.30))
        self.thresholds['jump_arm_raise'] = max(0.02, min(0.06, jump_arm_raise * 0.35))
        self.thresholds['duck_position'] = max(0.02, min(0.08, duck_head_drop * 0.30))
        self.thresholds['duck_hip_factor'] = 0.5
        self.thresholds['lean_right'] = max(0.03, min(0.10, lean_right_shift * 0.35))
        self.thresholds['lean_left'] = max(0.03, min(0.10, lean_left_shift * 0.35))
        self.thresholds['lean_velocity'] = 0.05

        print("[Calib] Dynamic thresholds set: jump_pos={:.3f} jump_arm={:.3f} duck_pos={:.3f} lean_r={:.3f} lean_l={:.3f}".format(
            self.thresholds['jump_position'], self.thresholds['jump_arm_raise'],
            self.thresholds['duck_position'], self.thresholds['lean_right'], self.thresholds['lean_left']))
    ...
    def interpret(self, result, timestamp):
        ...
        shoulder_y = self._smoother.shoulder_y
        hip_cy = self._smoother.hip_cy
        vel_up = self._smoother.velocity_up
        min_elbow_y = self._smoother.min_elbow_y

        # --- Calibration ---
        if self.baseline_y is None:
            self._update_calibration(
                head_y, shoulder_cx, shoulder_y, hip_cy, min_elbow_y
            )
            return Action.NONE
        ...
        shoulder_rise = self.baseline_shoulder_y - shoulder_y if self.baseline_shoulder_y is not None else 0.0
        arm_raise = shoulder_y - min_elbow_y if shoulder_y is not None else 0.0

        # Jump: velocity UP OR head above baseline OR arms raised over shoulders
        jump_vel_ok = vel_up > self.thresholds['jump_velocity']
        jump_pos_ok = dy > self.thresholds['jump_position']
        jump_arm_ok = arm_raise > self.thresholds['jump_arm_raise']
        if jump_vel_ok or jump_pos_ok or jump_arm_ok:
            action = Action.JUMP
        # Duck: head AND hips below baseline
        elif (head_y - self.baseline_y > self.thresholds['duck_position'] and
              hip_cy - self.baseline_y > self.thresholds['duck_position'] * self.thresholds['duck_hip_factor']):
            action = Action.DUCK
        # Lean: quick faint (velocity) OR significant position shift
        else:
            lean_vel_right = self._smoother.shoulder_cx_velocity > self.thresholds['lean_velocity']
            lean_vel_left = self._smoother.shoulder_cx_velocity < -self.thresholds['lean_velocity']
            lean_pos_right = shoulder_cx > 0.5 + self.thresholds['lean_right']
            lean_pos_left = shoulder_cx < 0.5 - self.thresholds['lean_left']

            if lean_vel_right or lean_pos_right:
                action = Action.RIGHT
            elif lean_vel_left or lean_pos_left:
                action = Action.LEFT
        ...
        if action != Action.NONE:
            ...
            if action == Action.JUMP:
                print("[Action] JUMP  (vel={:.3f}, dy={:.3f}, arm={:.3f}, shoulder_rise={:.3f})".format(
                    vel_up, dy, arm_raise, shoulder_rise))
            elif action == Action.DUCK:
                print("[Action] DUCK  (head={:.3f}, hip={:.3f})".format(head_y, hip_cy))
            else:
                print("[Action] {} (shoulder_cx={:.3f}, vel={:.3f})".format(
                    action.name, shoulder_cx, self._smoother.shoulder_cx_velocity))

        return action

    def calibration_progress(self):
        """Return (elapsed_seconds, total_seconds, done_bool, stage_name, stage_index)."""
        if self.baseline_y is not None:
            return Config.CALIB_STAGE_DURATION, Config.CALIB_STAGE_DURATION, True, "Done", len(self.calibration_stages)
        if self._calib_start is None:
            return 0.0, Config.CALIB_STAGE_DURATION, False, self.calibration_stages[0] if self.calibration_stages else "", 0
        # Calculate based on current stage
        stage_index = self._calib_stage_index
        stage_name = self.calibration_stages[stage_index] if stage_index < len(self.calibration_stages) else "Done"
        stage_start = self._calib_start_stage if self._calib_start_stage else self._calib_start
        elapsed = time.time() - stage_start
        total = Config.CALIB_STAGE_DURATION * len(self.calibration_stages) if not self.baseline_y else Config.CALIB_STAGE_DURATION
        return elapsed, total, False, stage_name, stage_index
...
class KeySender:
    def __init__(self, config):
        ...
        if self._backend == "none" and YDO_TOOL_OK:
            # Check if ydotoold daemon is actually running/responding
            daemon_ok = False
            try:
                # A quick ping to see if ydotoold responds (use same socket env)
                env_ping = os.environ.copy()
                env_ping["YDOTOOL_SOCKET"] = "/run/user/{}/.ydotool_socket".format(os.getuid())
                subprocess.run(
                    ["ydotool", "key", "0"],
                ...
                try:
                    subprocess.Popen(
                        ["ydotoold", "--socket-path=/run/user/{}/.ydotool_socket".format(os.getuid())],
                        stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                        start_new_session=True
                    )
                    print("[KeySender] Started ydotoold daemon.")
                    daemon_ok = True
                except Exception as e2:
                    print("[KeySender] Could not start ydotoold: {}".format(e2))

            ydotool_env = os.environ.copy()
            ydotool_env["YDOTOOL_SOCKET"] = "/run/user/{}/.ydotool_socket".format(os.getuid())
            if daemon_ok:
                try:
                ...
                # Try one more time after daemon start attempt
                try:
                    subprocess.run(["ydotool", "key", "0"],
                                  capture_output=True, timeout=2,
                                  env=ydotool_env)
                    self._backend = "ydotool"
                    print("[KeySender] Using ydotool backend (after daemon start).")
                except Exception as e:
                    print("[KeySender] ydotool still not working after daemon start:", e)
    ...
    def test_keypress(self):
        """Send a test 'up' arrow to verify the key backend works."""
        print("[KeySender] Testing keyboard backend ('{}')...".format(self._backend))
        result = self.send_action(Action.JUMP)
        print("[KeySender] Test keypress result: {}".format("SENT" if result else "FAILED"))
        return result
    ...
    def _send_ydotool(self, arrow):
        ...
        code = code_map[arrow]
        env = os.environ.copy()
        # Use the same socket path the daemon was started with
        env["YDOTOOL_SOCKET"] = "/run/user/{}/.ydotool_socket".format(os.getuid())
        subprocess.run(
...
class HUD:
    ...
    def draw(self, frame, focus, action, fps, backend, interpreter):
        ...
        elapsed, total, done, stage_name, stage_index = interpreter.calibration_progress()
        if not done:
            # Show stage name prominently
            stage_text = stage_name
            # Progress bar for current stage
            prog = min(elapsed / Config.CALIB_STAGE_DURATION, 1.0)
            bar_w = int(w * 0.6)
            bar_x = int(w * 0.2)
            bar_y = h // 2 - 20
            ...
            cv2.rectangle(frame, (bar_x, bar_y),
                         ...
                         (0, 180, 255), -1)
            # Instruction text
            instruction = stage_text
            cv2.putText(frame,
                         instruction,
                         (bar_x, bar_y - 30),
                         self._font, 0.55, (0, 180, 255), 2, cv2.LINE_AA)
            # Timer
            cv2.putText(frame,
                         "Stage {}/{} — {:.0f}s / {:.0f}s".format(
                             stage_index + 1, len(Config.CALIB_STAGES),
                             min(elapsed, Config.CALIB_STAGE_DURATION), Config.CALIB_STAGE_DURATION),
                         (bar_x, bar_y - 8),
                         self._font, 0.5, (255, 255, 255), 2, cv2.LINE_AA)
...
class Application:
    ...
    def run(self):
        ...
        print("  4. When the window opens, follow the 5 guided calibration stages.")
        print("     Each stage lasts {:.0f}s — stand, jump, duck, lean right, lean left.".format(
            Config.CALIB_STAGE_DURATION))
        print("  5. Press 'c' at any time to re-calibrate.")
        print()
        print("Calibration will guide you through 5 stages:")
        for i, stage in enumerate(Config.CALIB_STAGES):
            print("  {}. {}".format(i + 1, stage))
        print("  Each stage lasts {:.0f}s. Press 'c' at any time to re-calibrate.".format(
            Config.CALIB_STAGE_DURATION))
        # Show keyboard / display info
        print("[Config] DISPLAY env: {}".format(os.environ.get("DISPLAY", "NOT SET (pynput may fail)")))
        print("[Config] Key backend: {}".format(self.sender._backend))
        print()
